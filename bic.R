...
for (thr in seq(1.0, 0.1, by = -0.1)) {
  
  threshold_cs <- thr
  threshold_ucs <- thr
  
  cat("Processing with threshold_cs =", threshold_cs, 
      "and threshold_ucs =", threshold_ucs, "\n")

  Data <- read.csv("final_valid.csv", header=T)
  
  source_python("CS_UCS.py")
  
  Data$is_geneEX1 <- Data$ex1_estimate >= 0
  Data$CS_EX1 <- sapply(1:nrow(Data), function(i) CS(c(Data$ex1_a[i], Data$ex1_b[i], Data$ex1_c[i], Data$ex1_d[i]), threshold_cs, is_gene=Data$is_geneEX1[i]))
  Data$UCS_EX1 <- sapply(1:nrow(Data), function(i) UCS(c(Data$ex1_a[i], Data$ex1_b[i], Data$ex1_c[i], Data$ex1_d[i]), threshold_ucs, is_gene=Data$is_geneEX1[i]))
  
  Data$is_geneEX2 <- Data$ex2_estimate >= 0
  Data$CS_EX2 <- sapply(1:nrow(Data), function(i) CS(c(Data$ex2_a[i], Data$ex2_b[i], Data$ex2_c[i], Data$ex2_d[i]), threshold_cs, is_gene=Data$is_geneEX2[i]))
  Data$UCS_EX2 <- sapply(1:nrow(Data), function(i) UCS(c(Data$ex2_a[i], Data$ex2_b[i], Data$ex2_c[i], Data$ex2_d[i]), threshold_ucs, is_gene=Data$is_geneEX2[i]))
