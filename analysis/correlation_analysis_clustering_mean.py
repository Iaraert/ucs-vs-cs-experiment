...
class CorrelationAnalyzer:
    ...
    def _safe_score(self, func, counts, threshold, is_gene, label: str, **kwargs) -> float:
        ...
        try:
            with np.errstate(divide='ignore', invalid='ignore', over='ignore'):
                # CS関数とUCS関数の両方に対応する引数の順序
                score = func(counts, threshold=threshold, is_gene=is_gene, **kwargs)
        except Exception as exc:
            if self.debug:
                print(f"Warning: {label} calculation failed: {exc}")
            return np.nan
        if not np.isfinite(score):
            if self.debug:
                print(f"Warning: {label} returned non-finite value: {score}")
            return np.nan
        return float(score)
    ...
    def metrics_from_abcd(
        ...
        if cond_processed == 0:
            ...
            cs_single = self._safe_score(CS, (a, b, 0, 0), cs_th, is_gene, "CS_single")
            ucs_single = self._safe_score(UCS, (a, b, 0, 0), ucs_th, is_gene, "UCS_single")
    ...
    def _plot_cs_ucs_scatter(
        ...
        ).join(model_df[["CS", "UCS"]], how="inner")
        scatter_df = scatter_df.dropna(subset=["estimate_mean", "CS", "UCS"])
        if scatter_df.empty:
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    analyzer = CorrelationAnalyzer("final_valid_6_samples.csv", max_k=5, debug=False)
